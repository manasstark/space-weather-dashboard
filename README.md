...
* The confidence heuristic's calibration (`swdss.engine.confidence`) still does not hold up cleanly for the headline (1h/interval) forecasts, now with a much larger sample than the original read: Very High 74.6% (n=67), High 70.8% (n=65), Moderate 80.4% (n=51), Low 81.25% (n=16). Very High now correctly beats High (the original day-one inversion between those two has resolved), but Moderate and Low still both outperform Very High and High — a well-calibrated score should decrease monotonically top to bottom, and this doesn't. Reported honestly rather than hidden (see Verification tab → Confidence Calibration); not yet root-caused. With over four times the original sample size showing the same qualitative pattern, this now looks more like a real property of the heuristic's weighting than a small-sample artifact — worth investigating directly rather than waiting for more data to resolve it on its own.
...
* **Site header redesigned as a single-row, traditional-website nav bar** (2026-07) — previously the masthead (title/date) and the dark nav bar were two separate blocks, and Refresh/Space Weather Concepts/References sat in their own plain button row underneath the nav entirely. Restructured so the page links and the three action buttons now share one continuous dark bar (`st.columns` inside the same `topnav` container, buttons restyled as ghost buttons via `style_top_nav` so they read as part of the bar rather than mismatched white Streamlit buttons floating below it) — all 7 page links plus all 3 buttons fit on one line with zero wrapping or overlap, verified via direct DOM measurement (`scrollWidth == clientWidth`, zero overflow) as well as visually. Getting there surfaced two real, non-obvious CSS bugs worth recording: (1) `stColumn` defaults to `min-width: auto` (a standard flexbox gotcha), so a column will refuse to shrink below its content's natural width no matter how the ratio is tuned, unless explicitly forced to `min-width: 0`; (2) a flex container narrower than its children's combined width, combined with `justify-content: flex-end`, doesn't clip or wrap the overflow — it bleeds it in the opposite direction of the justification, invisibly overlapping whatever sits before it, unless the container also gets an explicit `overflow: hidden` backstop. Both are now documented directly in `style_top_nav`'s CSS comments.
  * **Known limitation, honestly unresolved**: the header was also meant to be sticky (pinned to the top of the viewport while scrolling, via `position: sticky` on the wrapping container) — the CSS is textbook-correct and `getComputedStyle` confirms `position: sticky` applied with no interfering `overflow`/`transform` on any ancestor between it and the scrolling container, yet it did not visually stay pinned when scroll-tested in this session's automated browser (it scrolled away like ordinary static content). Two targeted fixes (`align-self`, explicit `width`) were tried and reverted — one made sizing worse without fixing stickiness. This may be specific to the automated testing browser used this session rather than a real browser; it needs to be re-checked in an actual browser before relying on it. The single-row layout fix above is real and verified regardless of whether the sticky behavior holds up.
...
* **Verify the sticky header in a real desktop browser** — `position: sticky` on the combined masthead+nav bar didn't visually stay pinned when scroll-tested in this session's automated browser despite standards-correct CSS; needs a real-browser check (and likely a different fix, e.g. `position: fixed` with a computed left-offset matching the sidebar width) before it can be called done
...
* **Run the Bz/Kp Optimization Studies at least once each end-to-end** and review their promotion recommendations against production — AE's has now been run (see Development Roadmap above: no promotions, but a real crossover finding); Bz and Kp's have not yet been executed
...
* **Root-cause the headline confidence calibration inversion** once sample sizes are large enough to trust — is "High" confidence genuinely less reliable than "Moderate" for the 1h/interval forecasts specifically, or an artifact of which variables/horizons currently dominate that bucket?
* **A model that's nonlinear without losing extrapolation** — storm-phase hints, off-the-shelf nonlinear algorithms (RandomForest/XGBoost), and now a physically-motivated nonlinear equation (Burton et al. 1975's ring-current injection/decay ODE, both constant- and variable-decay-time versions — see Development Roadmap above) have all been tried; none is a clean win yet. The Burton hybrid came closest — it genuinely beat production on a real blind-test storm once variable decay time was added — but still lost clearly on the most extreme event in this project's set (May 2024 Gannon, G5). Remaining candidates not yet tried: a nonlinear/saturating injection term (not just variable τ) specifically for the most extreme driving levels, and testing the physics approach at longer horizons (6h/12h/24h) where production's own recent-history features carry less signal and genuine extrapolation may matter more than it does at the 1h horizon tested so far
* **Storm-aware model selection criterion** — `_fit_best` currently selects the winning algorithm by average CV R² across mostly-quiet history, so a model that's mediocre during storms but great on average still wins; explicitly reporting (and optionally weighting) storm-period performance during model selection — reusing the Storm-vs-Quiet metric the Hypothesis Testing framework already computes — would let a genuinely storm-robust algorithm actually get chosen
* **Satellite Operator module — proposed, not started** (2026-07) — a new domain layer (thermospheric density → drag → per-satellite orbit error) targeting a real, underserved commercial niche (see the July 2026 market intelligence briefing this was scoped against). Proposed architecture reuses the exact physics-baseline-plus-residual-ML pattern already proven by the Burton Dst hybrid above, phased so the core science (does it beat the standard NRLMSIS 2.1 baseline specifically during storms) is validated before any new heavy dependency (Orekit, for orbit propagation) is taken on. Full phased proposal written up; no code started.
* Public deployment
