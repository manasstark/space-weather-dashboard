...
`tests/` covers the physics formulas (VBz, Ey, Dynamic Pressure, Newell Coupling, Akasofu ε, Boyle Index, Clock Angle, Southward Duration, etc.), the lag/rolling/rate-of-change feature engineering, and the dataset registry — all pure, deterministic logic with no dependency on live data, network access, or trained models, so the suite runs in under a second. A `py_compile`-based syntax check also guards `dashboard/home.py` and every `dashboard/lib/*.py` module (see below) against a broken import or typo, without needing a live Streamlit runtime to do it. `.github/workflows/ci.yml` runs both on every push/PR to `main`.
...
* **Storm-phase-aware physics features on production Dst/AE** — with both raw storm-row addition and storm-weighted training now ruled out (see Storm-hour sample weighting above), this is the more promising remaining lever: the Kp Research Lab already prototypes Storm Phase, Time Since Southward Turning, and Previous Storm Strength as physics-experiment features, but they've never been tried on the actual production Dst/AE feature set specifically against Storm Backtest's storm-period accuracy
* **A model with real nonlinear capacity for the storm regime** — Dst's winning algorithm is LinearRegression across every weighting tried; a single global linear fit has no structural way to represent nonlinear storm dynamics (ring current saturation, threshold effects) no matter how training data is reweighted. Worth testing whether RandomForest/XGBoost — already candidates, just never selected — do better specifically on storm-period accuracy even if their average (quiet-dominated) R² is slightly lower, which ties directly into the model-selection-criterion idea below
