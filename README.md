...
**"Matches training preprocessing exactly" didn't fully hold for the 1h/interval headline slots specifically** — the last resampled hour can still be currently forming when a continuously-reissued job is created, not yet the complete 60-minute mean training saw. Root-caused and a fix shipped; see [Known Limitations](#known-limitations) and Development Roadmap → "1h/interval headline forecasts root-caused... and a train/serve input-freshness fix shipped."
...
* **Most headline (1h/interval) forecasts still show negative skill vs. persistence — improving, not yet resolved.** A first fix (2026-08-22, "v1") looked promising at n=11-12 evaluations/variable, but a second checkpoint at n=23 reversed it — every variable's win-rate dropped, including the one (Bz) that had genuinely beaten persistence. Investigating that reversal found v1's actual flaw: it let persistence keep reading fresher data than the model was fed for the same forecast, and separately created a 2-hour data-to-target gap the model was never trained for. A corrected version ("v2") shipped 2026-08-24 (see Development Roadmap → "1h/interval headline forecasts root-caused...") and its first checkpoint (2026-08-25, n=11-12) shows **4 of 9 variables now genuinely beating persistence (Speed, Dst, Bz, Kp)** — full numbers in `docs/1h_forecast_fix_soak_test_log.md`. Given v1 looked just as promising at this same sample size before reversing at n=23, this is reported as an encouraging first checkpoint, not a validated fix — the real test is whether these 4 wins hold up past n≈23. Kp's own prediction model (`predict_kp_interval`) remains completely untouched by either fix — its improvement so far is entirely from the persistence-anchor correction — and still needs its own dedicated investigation.
...
**Phase 0 progress so far — access and tooling verified real, nothing built yet.** Before writing any modeling code, confirmed every dependency Phase 0 needs actually works, same discipline as everything else in this project:
* The MIT STORM-AI benchmark dataset is genuine, public, and openly licensed — `doi:10.7910/DVN/U6K6MJ` on Harvard Dataverse, CC BY 4.0, no login required. Queried the Dataverse API directly for the full file listing rather than trusting the docs at face value: 18 files across `phase_1/` (training) and `private_eval/` (held-out satellite archives — CHAMP, GRACE, GRACE-FO, SWARM — for Phase 1, not Phase 0).
* `pymsis` (NRLMSIS 2.1) installs cleanly and produces physically correct output, verified rather than assumed: ~4.8×10⁻¹³ kg/m³ at 500km, the right order of magnitude for real thermospheric density, computed from live-fetched real space-weather indices (Celestrak), not placeholder input.
* Downloaded and schema-verified the core `phase_1/` data needed for a first pass — `initial_states.zip` (orbital elements per sample), `sat_density.zip` (the actual 3-day, 10-minute-cadence density target), `omni2.zip` (solar wind drivers) — roughly 1GB combined, into `data/storm_ai/` (already covered by this repo's existing `/data/` gitignore). Deliberately did **not** pull `goes.zip` (14.7GB) or any `private_eval/` satellite archive (tens more GB combined) — GOES X-ray flux is supplementary context, not the core driver, and OMNI2 alone already carries what this project's existing feature pipeline is built around; pulling multi-gigabyte files nothing yet needs would have been exactly the kind of waste this project's own retention/storage discipline elsewhere argues against.
* Deliberately stopped here rather than committing to a specific residual-model architecture solo — how to define "storm-regime hours" for the go/no-go gate, and whether to reuse the existing feature-engineering code as-is or adapt it for this different target, are real design decisions worth a second pair of eyes before locking in, not something to guess through alone.
...
* **1h/interval headline forecasts root-caused for "No Skill Over Persistence," and a train/serve input-freshness fix shipped** (2026-08) — every one of the 10 headline (dataset, variable, horizon=1h/interval) forecasts was scoring "No Skill Over Persistence" in `skill_scores.json` — Kp worst (skill −4.90, beats persistence 5/158 times), Density −4.07, down to Dst least-bad at −0.28 (still negative) — while every one of the same variables' 3h/6h/12h/24h models showed a clean, monotonic *recovery* into positive skill (e.g. Density: −4.07 at 1h → +0.70 at 24h; Bz: −0.57 → +0.50). That recovery shape is the actual diagnostic signal: if 1h were simply an intrinsically hard baseline to beat, the numbers wouldn't cleanly flip sign as horizon grows — something specific to the *shortest* horizon was degrading. Root cause, found by reading the shared prediction path (`predict.py`/`jobs.py`) rather than assumed: `swdss.models.train._load_base_df` trains every model exclusively on complete, fully-elapsed 60-minute hourly means, but `continuous_reissue` (see "Continuous re-issue for 1h/Kp jobs" above) means a fresh 1h/interval job is often created within a minute of a new hour starting — so `predict.load_live_features`'s last resampled row, fed live to the model as "the current value," is frequently built from only 1-2 minutes of real data, not a real hour's worth. The trained model's learned coefficients amplify that noise into real prediction error; the persistence baseline (`orchestrator._resolve_persistence_anchor`) does zero transformation on the same noisy reading, so it isn't hurt the same way — which is why persistence looked artificially strong by comparison rather than genuinely stronger.
  * **The fix, v1 (superseded — see below)** (`predict.py`'s new `_latest_complete_hour` helper): when the model's own input row is still a currently-forming hour, fall back to the last genuinely complete hour's values for the actual regression input — matching what training saw. Deliberately scoped to the model's input only, not the persistence baseline (kept reading whatever's freshest, on the theory that a naive "assume no change" guess doing zero transformation on noisy data is already a fair comparison) and not Kp (`predict_kp_interval` is a separate, non-shared code path — left for its own follow-up given how much worse its score is than everything else).
  * **A real bug caught and fixed during implementation, not after**: the first version of this fix let the fallback row's own timestamp become `observed_at`, which — since `predicted_for = observed_at + horizon` becomes the job's actual target hour (`jobs.start_job`) — silently *shortened the real forecast lead time* (a job created at 17:05 would anchor to 16:00 and target 17:00, a horizon that's already in progress, not 18:00). Corrected so `observed_at` always stays the true latest available moment regardless of which row's values get used for the model's actual input — timing and data-quality are two independent concerns that the first draft had conflated into one return value.
  * Verified in isolation (three synthetic cases: mid-forming hour correctly falls back; a single-row cold start can't fall back and doesn't crash; an already-complete last bucket is used directly with no unnecessary fallback) and against real production data structures with no errors.
  * **First real soak-test checkpoint** (2026-08-22, ~11.75h into the fixed run, n=93 fresh 1h/interval evaluations — full numbers in `docs/1h_forecast_fix_soak_test_log.md`): win-rate (fraction of hours the model beat persistence) improved for 6 of 8 non-Kp variables, some substantially (Bz 34.0%→63.6%, By 40.1%→54.5%, Bt 29.0%→45.5%), flat for Density/Dst. **On the stricter mean-error basis, only Bz currently beats persistence outright** (0.961 vs. 0.992 MAE) — every other variable still shows higher average error than persistence even under the fix. Reported as a checkpoint, not a verdict: n=11-12 per variable is still small enough for a couple of bad predictions to swing the mean, and the entire run has been during geomagnetically quiet conditions, which makes "assume no change" an unusually strong baseline regardless of model quality — both likely still masking some of the real effect. Kp remains unchanged as expected (0/4 on this fresh window) — separate code path, not addressed by this fix. **This checkpoint reversed by n=23 (see v2 below) — do not treat it as validated.**
  * **v2 first checkpoint** (2026-08-25, n=11-12/variable under v2 — see Known Limitations and `docs/1h_forecast_fix_soak_test_log.md` for the v1-reversal analysis and full v2 fix description): **4 of 9 headline variables now beat persistence outright** (Speed, Dst, Bz, Kp), up from 0-1 at every prior checkpoint. Notably Kp improved despite its own model (`predict_kp_interval`) being untouched by either fix — only its persistence anchor changed — which cross-checks that the anchor-fairness theory was genuinely part of the problem, not coincidence. Same caution applies: v1 looked similarly promising at this same sample size before reversing at n=23, so this is reported as the first encouraging checkpoint, not a validated result.
  * **Second checkpoint reversed the early signal, and surfaced why v1 was actually wrong** (2026-08-24, n=23/variable, ~double the first checkpoint): every variable's win-rate *dropped* versus the first checkpoint, and Bz — the one genuine win at n=11-12 — no longer beat persistence at all (63.6%→47.8% win-rate, 0.961→0.930 model MAE now worse than persistence's 0.890). The early "improvement" was mostly noise from a small sample regressing back down, not a real trend. Investigated rather than assumed: v1 kept `observed_at` pinned to the true latest hour while only the *feature row* fell back to the prior complete hour — meaning the model was reasoning from data an hour older than its own stated timestamp, while still being asked to predict a FULL hour past the *current* hour. That's a real gap of 2 real hours between the data used and the target, which training never saw (training only ever saw exactly 1-hour gaps) — trading "noisy but current" for "clean but stale and over-extrapolated," not a net fix. Confirmed live: a job created at 18:06 was reasoning from 17:00's data while targeting 19:00. Worse, persistence's own anchor (`_resolve_persistence_anchor`, called with the job's real `created_at`) was untouched by v1, so it kept reading whatever was freshest — confirmed live, its anchor value at a real evaluation moment exactly matched the raw partial-hour value v1's own fix was built to avoid feeding the model. Persistence was quietly getting a full hour's freshness advantage over the model for the same forecast, which is not "zero-transformation on the same input" — it's a different, easier input.
  * **The fix, v2 (current)**: `_latest_complete_hour` now advances the feature row *and* its timestamp together — `observed_at` and `predicted_for` both fall back to the last complete hour, so the gap between the data used and the target is always exactly 1 hour, matching training exactly. `_resolve_persistence_anchor` now anchors to the same last-complete-hour definition (`created_at`'s own hour, floored, minus one hour) instead of whatever's freshest, so neither side gets a freshness advantage — verified live, the model's `current_value` and persistence's anchor now resolve to the identical number for the same forecast. Honest tradeoff: a forecast now only refreshes once per hour (right after the previous hour closes) instead of within the first few minutes of a new one. Checked for a new risk before shipping this: does a target hour that's technically "in the past" relative to when the job was created let it resolve against an incomplete actual value? No — `jobs.py`'s own finalize logic already gates on `now >= target_hour + 1 hour` before ever calling `resolve_actual_value`, independent of anything this fix touches, so evaluation still always waits for genuine hour completion either way. Not yet soak-tested; the next checkpoint is the real test of whether v2 actually holds up.
* **Reports feature — a single downloadable file replacing the old raw-file Downloads tab** (2026-08-22) — the Downloads tab previously listed ~9 raw operational files (forecast snapshots, package history, evaluation history, skill scores, calibration report) with no explanation of what any of them meant. Replaced with one thing: `history/hourly_report.parquet` (`storage.append_report_rows`), one row per (variable, hour) written the moment that hour's headline forecast gets a real observed value to grade against — predicted, actual, error, confidence, a persistence comparison, and a plain-language problem flag (`low_confidence`, `underperformed_persistence`). Rolling 7-day window (`REPORT_HISTORY_RETENTION_DAYS`, matching NOAA's own "today + last N days" convention), with re-evaluations of the same hour replacing rather than duplicating (`drop_duplicates` on `(variable, horizon, valid_end)`). The Downloads tab now groups by calendar day (`21 Aug 2026 — ran 4 of 24 expected hours`), computed directly from which hours actually have data — no separate uptime tracking needed, since a data gap already means "the engine wasn't producing forecasts that hour." A trailing gap specifically (the single most recent hour boundary) gets its own explicit note (`Hour ending 18:00 UTC still evaluating — not yet included below`) rather than being silently indistinguishable from a real miss — only the literal most recent boundary gets this treatment; an interior gap is still reported as a real gap. Raw operational files are unchanged on disk (the engine's other tabs still depend on them) — they're just no longer surfaced as downloads, since they were never meant to be read directly by a human.
  * **PDF export** (`dashboard/lib/report_pdf.py`) — built entirely with matplotlib (already a dependency, no new library added): a plain, monospace, dash-ruled document — title/coverage summary, one page of predicted-vs-actual charts per variable (all headline variables fit on a single page regardless of count, pinned to the real report window rather than trusting matplotlib's date-axis autoscale, which picks a nonsensical multi-year range with only 1-2 data points), then one paginated table per day. Both CSV and PDF build lazily on click (same pattern already used for CSV elsewhere in this tab, added specifically after an earlier bug where an eager `st.download_button` silently re-executed an expensive multi-million-row export on every single rerun) — which also means the PDF's "still evaluating" note reflects genuinely live state at the moment of download, not a stale pre-baked snapshot. Two real bugs found and fixed during the build itself, both visible directly in the rendered output: matplotlib's `ax.table` doesn't auto-size column widths, so the VS. PERSISTENCE/FLAGS columns overlapped illegibly until given explicit width ratios; and the chart x-axis bug just described.
* **`flare_cme_predict.py` crash on zero scorable SHARP regions, fixed** (2026-08-22) — `orchestrator._flare_cme_outlook_and_lean` filters active regions down to ones with a full 24h of SHARP feature history (`ready_rows = latest_per_ar.dropna(subset=FEATURE_COLUMNS)`) before scoring them, and this can legitimately produce zero rows (e.g. every currently-active region is younger than 24h) — but `predict_flare_probability`/`predict_cme_probability` only checked whether the trained model file existed, never whether there was actually anything to score, so `RandomForestClassifier.predict_proba` on a 0-row array raised instead of returning cleanly. This took down the *entire* Solar Forecast snapshot for that cycle, not just the flare/CME section, since the crash happened before the function could return. Fixed with an explicit `latest_features.empty` guard returning `None` (already the exact case `_top_region_summary` handles as `"no_scorable_regions"` — the dashboard already had the right message wired up, it just never got a chance to render). Reproduced the exact crash directly against a synthetic 0-row frame before and after the fix to confirm.
* **Streamlit `cache_data`/`cachetools` KeyError race, fixed dashboard-wide** (2026-08-22) — a known Streamlit internals bug: its in-memory cache storage can evict an entry (TTL expiry sweep) between the "is this key cached" check and the actual read, raising a bare `KeyError` from inside `cachetools` instead of transparently recomputing. Confirmed live, taking down the *entire* dashboard — `load_master_data` sits near the top of `home.py`, so the crash blocked everything below it from ever rendering. Fixed with `dashboard.lib.data_helpers.retry_on_cache_race`, a small decorator that retries the call once on `KeyError` (the race is a pure timing coincidence, not a real data problem, so an immediate retry either hits a freshly-rewritten cache entry or recomputes cleanly) — applied to every `@st.cache_data`/`@st.cache_resource`-decorated function across the dashboard (`data_helpers.py`, `command_centre.py`, and all three Research Lab modules), not just the one that happened to crash first, since the same race is latent in any of them. Unit-tested directly: retries exactly once and succeeds, gives up cleanly rather than looping forever if the error persists, never swallows an unrelated exception type, and leaves a normal successful call untouched.
