...
* **All 10 headline (1h/interval) forecasts currently show negative skill vs. persistence** — still true as of this writing, and not yet resolved. A first fix (2026-08-22) looked promising at n=11-12 evaluations/variable, but a second checkpoint at n=23 reversed it — every variable's win-rate dropped, including the one (Bz) that had genuinely beaten persistence. Investigating that reversal found the first fix's actual flaw: it let persistence keep reading fresher data than the model was fed for the same forecast, and separately created a 2-hour data-to-target gap the model was never trained for. A corrected version (v2) shipped 2026-08-24 — see Development Roadmap → "1h/interval headline forecasts root-caused..." for the full mechanism — but it is **not yet soak-tested**; given the first fix's early-checkpoint result didn't hold up, the next real checkpoint is what actually matters here, not a re-assertion that this one is right. Kp is the worst case by far (skill −4.90, beats persistence only 3% of the time) and goes through a separate code path (`predict_kp_interval`) neither fix has touched — it still needs its own dedicated look.
